...
| enableSIPInspection | bool | `true` | Enable the SIP datapath extensions (Call-ID conntrack keys, SIP timeouts and SIP-specific LB/NAT). Drain SIP traffic before changing this agent startup setting; existing conntrack/NAT entries are not migrated. |

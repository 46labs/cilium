...
// TestPrivilegedCtGcPortFilter tests that only TCP SNAT entries with specified port in GC filter are removed upon a removal of
// their CT entry.
func TestPrivilegedCtGcPortFilter(t *testing.T) {
	setupCTMap(t)
	// Init maps
	natMap := nat.NewMap(nil, "cilium_nat_any4_test", nat.IPv4, 1000)
	err := natMap.OpenOrCreate()
	...
	// Create the following entries and check that on of them get GC-ed:
	//	- CT:	TCP OUT 192.168.61.11:38193 -> 	192.168.61.12:80 <..>
	//	- NAT: 	TCP OUT 192.168.61.11:38193 -> 	192.168.61.12:80 	XLATE_SRC 192.168.61.11:38194
	//			TCP IN 	192.168.61.12:80 -> 	192.168.61.11:38194 XLATE_DST 192.168.61.11:38193

	//	- CT:	TCP OUT 192.168.61.11:39193 -> 	192.168.61.12:80 <..>
	//	- NAT: 	TCP OUT 192.168.61.11:39193 -> 	192.168.61.12:80 	XLATE_SRC 192.168.61.11:39194
	//			TCP IN 	192.168.61.12:80 -> 	192.168.61.11:39194 XLATE_DST 192.168.61.11:39193

	srcPort1 := uint16(0x3195)
	srcPort2 := uint16(0x1999)

	makeCtRecord(t, ctMap, srcPort1)
	makeCtRecord(t, ctMap, srcPort2)

	makeNatRecord(t, natMap, 0x3295, srcPort1)
	makeNatRecord(t, natMap, 0x1A99, srcPort2)
	...
	filter := GCFilter{
		MatchIPs: map[netip.AddrPort]struct{}{
			netip.AddrPortFrom(types.IPv4{192, 168, 61, 11}.Addr(), 39193): {},
		},
	}
	mcast, next, complete := stream.Multicast[GCEvent]()
	mcast.Observe(context.Background(), NatMapNext4, func(err error) {})

...
func (cm *CrapManager) addService(ctx context.Context, svc *slim_corev1.Service, diff *diff) error {
	...
	if _, ok := annotation.Get(svc, annotation.ServiceRaw); !ok {
		log.DebugContext(ctx, "Remove services w/o RAW annotation")
		return cm.delService(ctx, svc, diff)
	}
...
func (cm *CrapManager) addEndpoint(ctx context.Context, ep *k8sTypes.CiliumEndpoint, diff *diff) error {
	...
	identityLabels := labels.NewLabelsFromModel(ep.Identity.Labels)
...
func (config *serviceMetadata) matchesPodLabels(epNamespace string, epLabels map[string]string) bool {
	if config.namespace != epNamespace {
		return false
	}
